...
class app(Frame):
	def __init__(self, mw):
		...
		BuilderFrame.columnconfigure( 0, weight=1)
                ...
                thx=ttk.Label(Thanks,text=u'''\n\n\n
                        ...
                        -----------------\n
                        修改版 by pcat | http://pcat.cnblogs.com/\n
                        ''')
                thx.grid()
		# Help SqlMAP
		...
		# self.sql_var = StringVar() #del
		# self.sqlEdit = ttk.Entry(queryLF) #del
		# self.sqlEdit.config(text="", textvariable = self.sql_var) #del
		# self.sqlEdit.grid(sticky = 'we', pady=5) #del
		self.sqlEdit = Text(queryLF,height=3) #add
		self.sqlEdit.grid(row=0,column=0,sticky='we',pady=5) #add
		scrollSqlEdit = ttk.Scrollbar(queryLF,orient=VERTICAL, command=self.sqlEdit.yview) #add
		scrollSqlEdit.grid(row=0,column=1, sticky='ns') #add
		self.sqlEdit['yscrollcommand'] = scrollSqlEdit.set #add
		...
		proxyF.columnconfigure(0,weight=1) #add
		# take query SqlMAP
		but = ttk.Button(BuilderFrame)
		but.config(text =u"构造命令语句",width = 12, command=self.commands) #modify 10->12
		#
		...
		self.entryD.grid(row=0,column=0, sticky='we', padx=100) #modify 30->100
		...
		self.entryT.grid(row=1,column=0,sticky='we', padx=100) #modify 30->100
		...
		self.entryC.grid(row=2,column=0, sticky='we', padx=100) #modify 30->100
		...
		# ##add BEGIN########################
		# proxy
		proxyLF = ttk.Labelframe(proxyF, text=u'代理',width="10")
		proxyLF.grid(row = 0, column =0, sticky='nw')
		proxyLF.columnconfigure(0, weight=1)
		#
		self.chkproxy = ttk.Checkbutton(proxyLF)
		self.chkproxy_var = StringVar()
		self.chkproxy.config(text = "", variable= self.chkproxy_var, onvalue= "on" ,
			offvalue = "off", command= self.chek_proxy)
		self.chkproxy.grid(row=0,column=0, sticky='w')
		# #
		self.schemeProxy = ttk.Combobox(proxyLF)
		self.schemeProxy_value = StringVar()
		self.schemeProxy.config(textvariable=self.schemeProxy_value, state='disabled',width="10")
		self.schemeProxy['values'] = ("http","https","socks4","socks5")
		self.schemeProxy.current(0)
		self.schemeProxy.bind('<<ComboboxSelected>>', self.chek_proxy)	
		self.schemeProxy.grid(row=0,column=1)
		#
		label01 = ttk.Label(proxyLF, text=u'://')
		label01.grid(row = 0, column=2)
		#
		self.addressProxy = ttk.Entry(proxyLF)
		self.addressProxy.grid(row =0,column=3)
		self.addressProxy.bind('<Button-3>',self.rClicker, add='')
		#
		label02 = ttk.Label(proxyLF, text=u':')
		label02.grid(row = 0, column=4)
		#
		self.portProxy = ttk.Entry(proxyLF)
		self.portProxy.config(width="10")
		self.portProxy.grid(row =0,column=5)
		self.portProxy.bind('<Button-3>',self.rClicker, add='')
		#
		proxyLF2 = ttk.Labelframe(proxyF, text=u'身份验证（用户名:密码）')
		proxyLF2.grid(row = 1, column =0, sticky='nw',pady="15")
		proxyLF2.columnconfigure(0, weight=1)
		#
		self.chkproxy2 = ttk.Checkbutton(proxyLF2)
		self.chkproxy_var2 = StringVar()
		self.chkproxy2.config(text = "", variable= self.chkproxy_var2, onvalue= "on" ,
			offvalue = "off",command= self.chek_proxy2)
		self.chkproxy2.grid(row=0,column=0, sticky='w')
		#
		self.usernameProxy = ttk.Entry(proxyLF2)
		self.usernameProxy.grid(row =0,column=1)
		self.usernameProxy.bind('<Button-3>',self.rClicker, add='')
		#
		label03 = ttk.Label(proxyLF2, text=u':')
		label03.grid(row = 0, column=2)
		#
		self.passwordProxy = ttk.Entry(proxyLF2)
		self.passwordProxy.grid(row =0,column=3)
		self.passwordProxy.bind('<Button-3>',self.rClicker, add='')
	...
	# ##add BEGIN #########
	# proxy
	def chek_proxy(self, *args):
		sql_Proxy = self.chkproxy_var.get()
		if sql_Proxy == "on" :
			self.schemeProxy.config(state = 'readonly')
			sqlProxy = ' --proxy="'+self.schemeProxy_value.get()+"://"+self.addressProxy.get()+":"+self.portProxy.get()+'"'	
		else:
			self.schemeProxy.config(state = 'disabled')
			sqlProxy = ""
		return sqlProxy
	def chek_proxy2(self, *args):
		sql_Proxy2 = self.chkproxy_var2.get()
		if sql_Proxy2 == "on" :
			sqlProxy2 = ' --proxy-cred="'+self.usernameProxy.get()+":"+self.passwordProxy.get()+'"'	
		else:
			sqlProxy2 = ""
		return sqlProxy2
	# ##add END ###########
	...
	def commands(self):
		...
		inject = target+self.chekParam()+z_param+self.chek_tam()+ \
		        ...
		        self.chekFing() + self.chekBanner() +self.chek_proxy()+self.chek_proxy2() #add proxy
		# self.sql_var.set(inject) #del
		self.sqlEdit.delete("1.0",END) #add
		self.sqlEdit.insert("1.0",inject) #add
	# GOGO!!!
	def injectIT(self):
		if (os.name == "posix"):
			# cmd = "xterm -hold -e python sqlmap.py" + self.sqlEdit.get() #del
			cmd = "xterm -hold -e python sqlmap.py " + self.sqlEdit.get('1.0',END) #add
		else:			
			# cmd = "start cmd /k python sqlmap.py" + self.sqlEdit.get() #del
			cmd = "start cmd /k python sqlmap.py " + self.sqlEdit.get('1.0',END) #add
		#Write last target
