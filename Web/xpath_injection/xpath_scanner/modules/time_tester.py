...
def step(time,              _netcon,    t,   _xpath, iterator):  # step
    ...
    while not tb_checker(_netcon, choose_tag(t, s), time):
        length = length + 1
        s = template(t, if_then(string_length(
            _xpath, length), reverse(iterator)))
    result = ""
    for _i in range(1, length + 1):
        f = True
        for _j in string_of_chars:
            s = template(t, if_then(
                substring(_xpath, _i, _j), reverse(iterator)))
            if tb_checker(_netcon, choose_tag(t, s), time):
                result = result + _j
                f = False
                break
...
def recursive_descent(time,      _netcon,    t,  _xpath, _deep, iterator):  # recursive_descent
    ...
    if amount != None:
        ...
        for i in range(1, amount + 1):
            s = tab + "<"
            find_node = tab + "</"
            result = step(time,   _netcon,    t,
                          name(N.E(_xpath, i)), iterator)
            if result != None:
            ...
            # if result != None:
            #    s = s+' xmlns="'+result+'"'
            result = argument_cycle(
                time, _netcon, t,  N.E(_xpath, i), iterator)
            if result != None:
                s = s + result
            s = s + ">"
            print s
            result = step(time,   _netcon,    t,
                          text(N.E(_xpath, i)), iterator)
            if result != None:
                print result
                print find_node + ">"
                continue
