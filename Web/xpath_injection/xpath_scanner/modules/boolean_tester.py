    ...
    if amount != None:
        ...
        for i in range(1, amount + 1):
            ...
            if result != None:
                ...
                print find_node + ">"
                continue
            result = comment_cycle(
                b, _netcon,    t,  N.E(_xpath, i), tab + '\t')
            result = proc_cycle(b, _netcon,    t,  N.E(_xpath, i), tab + '\t')
            recursive_descent(b,      _netcon,    t,
                              N.E(_xpath, i),  _deep + 1)
            print find_node + ">"
    return
################################################################################
...
def scan(_settings, _netcon, _doc):  # scan
    try:
        ...
        if _settings.available('b'):
            ...
            XPIsettings = bb_test(_settings.get('b'), _netcon, _t, _doc)
            if XPIsettings[0]:
                recursive_descent(_settings.get('b'), _netcon,
                                  XPIsettings[1], _doc, 0)
            if DEBUG:
                print XPIsettings
    except Exception, e:
        print("[\033[91mERROR\033[92m]: Unexpected termination"
