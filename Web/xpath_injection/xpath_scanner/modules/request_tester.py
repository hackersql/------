import sys, socket, time, errno, urllib
...
def text_cycle(             _netcon,        _proc,      _r, t,  path):          #text_cycle
    s0 = encode_for_uri(concat3(simple_count(text(path)),'{tag}TAG{tag}',text(path)))
...
def argument_cycle(         _netcon,        _proc,      _r, t,  path):          #argument_cycle
    ...
    i = 1                                                                       #init i
    while i <= int(amount):
        s0 = encode_for_uri(concat3(name(A.E(path,i)),'{tag}TAG{tag}',A.E(path,i)))
        s = template(t,more_then(doc(concat2(url(_r[0],_r[1]),s0))+nodes('')))
        s = choose_tag(t,s)
...
def proc_cycle(         _netcon,        _proc,      _r, t,  path,    tab):      #argument_cycle
    ...
    i = 1                                                                       #init i
    while i <= int(amount):
        s0 = encode_for_uri(concat3(name(P.E(path,i)),'{tag}TAG{tag}',P.E(path,i)))
        s = template(t,more_then(doc(concat2(url(_r[0],_r[1]),s0))+nodes('')))
        s = choose_tag(t,s)
...
def recursive_descent(      _netcon,        _proc,      _r, t,  path, _deep):   #recursive_descent
    ...
    N = NODE()
    s0 = encode_for_uri(concat3(simple_count(N.S(path)),'{tag}TAG{tag}',name(N.E(path,1))))
    s = template(t,more_then(doc(concat2(url(_r[0],_r[1]),s0))+nodes('')))
    s = choose_tag(t,s)
    ...
    while i <= int(amount):
        ...
        first_node = first_node + argument_cycle(_netcon,_proc,_r,t, N.E(path,i))
        print first_node+'>'
