    ...
    def _get_cursor(self):
        ...
        if threadData.hashDBCursor is None:
            try:
                ...
                errMsg = u"打开会话文件'%s'时发生错误('%s')" % (self.filepath, getSafeExString(ex))
    ...
    def retrieve(self, key, unserialize=False):
                ...
                for _ in xrange(HASHDB_RETRIEVE_RETRIES):
                    try:
                        ...
                        if any(_ in getSafeExString(ex) for _ in ("locked", "no such table")):
                            warnMsg = u"访问会话文件'%s'时出现问题('%s')" % (self.filepath, getSafeExString(ex))
                        ...
                        errMsg = u"访问会话文件'%s'时出现问题('%s')，" % (self.filepath, getSafeExString(ex))
                        errMsg += u"如果问题仍然存在，请使用`--flush-session`重新运行"
                        raise SqlmapConnectionException, errMsg
                    else:
                        break
        ...
        if retVal and unserialize:
            try:
                ...
                retVal = None
                warnMsg = u"在对会话密钥'%s'取消排序化值时出错，" % key
                warnMsg += u"如果问题仍然存在，请使用`--flush-session`重新运行"
                logger.warn(warnMsg)
                ...
                while True:
                    try:
                        ...
                        if not os.path.exists(self.filepath):
                            debugMsg = u"会话文件'%s'不存在" % self.filepath
                            logger.debug(debugMsg)
                            break

                        if retries == 0:
                            warnMsg = u"写入会话文件时出现问题('%s')" % getSafeExString(ex)
                            logger.warn(warnMsg)
