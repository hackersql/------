...
class DBMS_DIRECTORY_NAME:  # 数据库目录名
    ACCESS = "access"
    DB2 = "db2"
    FIREBIRD = "firebird"
    MAXDB = "maxdb"
...
class NULLCONNECTION:   # 空连接
    HEAD = "HEAD"
    RANGE = "Range"
    SKIP_READ = "skip-read"

class REFLECTIVE_COUNTER:   # 反射型
    MISS = "MISS"   # 失败
    HIT = "HIT"     # 成功
...
class REGISTRY_OPERATION:   # 注册表操作参数
    READ = "read"
    ADD = "add"
    DELETE = "delete"

class DUMP_FORMAT:  # 转储数据库格式
    CSV = "CSV"
    HTML = "HTML"
    SQLITE = "SQLITE"
...
class PAYLOAD:
    SQLINJECTION = {
                        ...
                        3: "内联查询",
                        4: "堆叠查询注入，可以同时执行多条语句的注入。",
                        5: "AND/OR 基于时间的盲注，即不能根据页面返回内容判断任何信息，用条件语句查看时间延迟语句是否执行（即页面返回时间是否增加）来判断",
                        6: "联合查询注入，可以使用union的情况下的注入",
                   }
...
class ADJUST_TIME_DELAY:    # 调整时间延迟
    DISABLE = -1
    NO = 0
    YES = 1
...
class NOTE:
    FALSE_POSITIVE_OR_UNEXPLOITABLE = "误报或不可利用"
