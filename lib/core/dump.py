...
class Dump(object):
    ...
    def _write(self, data, newline=True, console=True, content_type=None):
        ...
        try:
            ...
            errMsg = u"写入日志文件时发生错误 ('%s')" % getSafeExString(ex)
    ...
    def setOutputFile(self):
        ...
        try:
            ...
            errMsg = u"打开日志文件时发生错误 ('%s')" % getSafeExString(ex)
            raise SqlmapGenericException(errMsg)
    ...
    def currentUser(self, data):
        self.string(u"当前用户", data, content_type=CONTENT_TYPE.CURRENT_USER)

    def currentDb(self, data):
        if Backend.isDbms(DBMS.MAXDB):
            self.string(u"当前数据库(在 %s上没有实际使用)" % Backend.getIdentifiedDbms(), data, content_type=CONTENT_TYPE.CURRENT_DB)
        elif Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.PGSQL, DBMS.HSQLDB):
            self.string(u"当前模式(相当于 %s上的数据库)" % Backend.getIdentifiedDbms(), data, content_type=CONTENT_TYPE.CURRENT_DB)
        else:
            self.string(u"当前数据库", data, content_type=CONTENT_TYPE.CURRENT_DB)

    def hostname(self, data):
        self.string(u"主机名", data, content_type=CONTENT_TYPE.HOSTNAME)

    def dba(self, data):
        self.string(u"当前用户是否为DBA", data, content_type=CONTENT_TYPE.IS_DBA)

    def users(self, users):
        self.lister(u"数据库管理系统用户", users, content_type=CONTENT_TYPE.USERS)
    ...
    def dbs(self, dbs):
        self.lister(u"可用数据库", dbs, content_type=CONTENT_TYPE.DBS)
    ...
    def dbTables(self, dbTables):
        if isinstance(dbTables, dict) and len(dbTables) > 0:
            ...
            self.singleString(u"找不到表", content_type=CONTENT_TYPE.TABLES)
        else:
    ...
    def dbTableValues(self, tableValues):
                ...
                try:
                    ...
                    if not os.path.isdir(dumpDbPath):
                        try:
                            ...
                            warnMsg = u"无法创建转储目录 "
                            warnMsg += u"'%s' (%s). " % (dumpDbPath, getSafeExString(ex))
        ...
        if count > TRIM_STDOUT_DUMP_SIZE:
            warnMsg = u"由于表格大小，控制台输出将只截取最后的%d行" % TRIM_STDOUT_DUMP_SIZE
        ...
        if conf.dumpFormat == DUMP_FORMAT.SQLITE:
            ...
            logger.info(u"表 '%s.%s' 转储到sqlite3数据库 '%s'" % (db, table, replication.dbpath))
            ...
            msg = u"表 '%s.%s' 转储到 %s 文件 '%s'" % (db, table, conf.dumpFormat, dumpFileName)
            if not warnFile:
                logger.info(msg)
            else:
                logger.warn(msg)
    ...
    def dbColumns(self, dbColumnsDict, colConsider, dbs):
        ...
        for column in dbColumnsDict.keys():
            ...
            msg = "在以下数据库中找到 %s 列:" % colConsiderStr
            self._write(msg)
