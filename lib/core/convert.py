    ...
    try:
        ...
        if IS_WIN:
            ...
            if '?' in output and '?' not in data:
                warnMsg = u"无法在Windows操作系统命令提示符"
                warnMsg += u"(http://bugs.python.org/issue1602)"
                warnMsg += u"内正确显示Unicode字符，"
                warnMsg += u"所有未处理的事件都将导致用'?'替换无法显示的字符"
                warnMsg += u"请在相应的输出文件中找到正确的字符表示。"
                singleTimeWarnMessage(warnMsg)
