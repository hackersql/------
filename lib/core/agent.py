...
class Agent(object):
    ...
    def prefixQuery(self, expression, prefix=None, where=None, clause=None):
        """
        此方法定义如何转义输入表达式以根据标识为有效的注入类型执行注入
        """
        ...
        # 如果使用堆叠(多语句)技术查询，不要在前缀之后放置一个空格
        # 或者它在GROUP BY/ORDER BY (<clause>)
    ...
    def adjustLateValues(self, payload):    # 延迟注入的时间
        """
    ...
    def hexConvertField(self, field):
        ...
        if 'hex' in rootQuery:
            ...
            warnMsg = "%s数据库目前不支持“--hex”选项" % Backend.getIdentifiedDbms()
    ...
    def nullAndCastField(self, field):
        """
        输入一个字段字符串并返回其处理后的nulled和casted字段字符串。
...
        注意：主要用于转储自定义表
    ...
    def concatQuery(self, query, unpack=True):
        """
        输入一个查询字符串并返回其处理为null，转换和连接的查询字符串。
    ...
    def forgeUnionQuery(self, query, position, count, comment, prefix, suffix, char, where, multipleUnions=None, limited=False, fromTable=None):
        """
        输入一个查询（伪查询）字符串并返回其处理的UNION ALL SELECT查询。
        ...
        @param query: 它是在UNION ALL SELECT语句中被伪造的被处理的查询字符串
        @type query: C{str}

        @param position: 注入查询的NULL位置
        @type position: C{int}

        @return: 伪造UNION ALL SELECT查询字符串
        @rtype: C{str}
        """
    ...
    def limitCondition(self, expression, dump=False):
        ...
        # 我们假设只有不包含“LIMIT＃，1”（或等同于后端DBMS的查询）的查询可以返回多个条目
        if limitCond:
            if (limitRegExp or limitRegExp2) and stopLimit is not None:
                stopLimit = int(stopLimit)

                # 从现在开始，我们只需要表达式直到“LIMIT”（或等效的，取决于后端DBMS）字
                if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.PGSQL, DBMS.SQLITE):
