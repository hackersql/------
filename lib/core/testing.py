...
def liveTest():
    ...
    for case in livetests.getElementsByTagName("case"):
        ...
        msg = u"运行实时测试用例: %s (%d/%d)" % (name, count, length)
        logger.info(msg)
        ...
        if result is True:
            ...
            if _failures.failedItems:
                errMsg += u"在解析项目: %s" % ", ".join(i for i in _failures.failedItems)

            errMsg += u" - scan folder: %s" % paths.SQLMAP_OUTPUT_PATH
            errMsg += u" - traceback: %s" % bool(_failures.failedTraceBack)

            if not vulnerable:
                errMsg += u" - 未检测到SQL注入"

            logger.error(errMsg)
            test_case_fd.write(u"%s\n" % errMsg)
    ...
    if retVal:
        logger.info(u"实时测试最终结果: 通过")
    else:
        logger.error(u"实时测试最终结果: 失败")
