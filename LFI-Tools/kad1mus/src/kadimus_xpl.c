...
int rce_scan(GET_DATA *GetParameters, size_t p_len, const char *base_uri, int p){
	...
	*php_code = NULL, *rce_uri = NULL;
	...
	if(!ret) print_single("[-] 测试显示不存在攻击漏洞，请手动测试！！！\n");
	print_single("[*] php://input 测试完成\n\n");
	...
	if(!ret) print_single("[-] 测试显示不存在攻击漏洞，请手动测试！！！\n");
	print_single("[*] /proc/self/environ 测试完成\n\n");
	...
	if(!HttpRequest(curl)){
		print_single("[-] 请求错误\n");
		print_single("[-] 测试显示不存在攻击漏洞，请手动测试！！！\n");
	} else {
		if( regex_match(regex, body.ptr, 0, 0) ){
	...
	print_single("[+] /var/log/auth.log 测试\n");
	char *mmap_str = NULL;
	//struct stat s;
	//int size_porra,fd;
	char random_file[20];
	...
	if(!ret) print_single("[-] 测试显示不存在攻击漏洞，请手动测试！！！\n");
        print_single("[*] /var/log/auth.log 测试完成\n\n");
...
void scan(const char *target_uri){
	...
	print_all("[+] %s\n\n",target_uri);
	print_all("[*] 测试URI是否具有动态内容\n");
	result = is_dynamic(target_uri);

	if(result == -1){
		print_all("[-] 与目标URI没有建立连接，退出 ...\n\n");
		goto end;
	}
