...
class Search:
    ...
    def searchTable(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            errMsg = u"information_schema表不可用, 后端DBMS是MySQL < 5.0"
            bruteForce = True

        if bruteForce:
            message = u"您是否希望使用公共表的检查存在性? %s" % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS,) else "[y/N/q]")
    ...
    def search(self):
        ...
        if conf.col:
            ...
            errMsg = "缺少参数，提供-D，-T或-C以及--search"
