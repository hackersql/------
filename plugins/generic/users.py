...
class Users:
                ...
                for user in users:
                    ...
                    if Backend.isDbms(DBMS.INFORMIX):
                        ...
                        if not isNumPosStrValue(count):
                            warnMsg = u"无法检索用户'%s'的密码哈希数量" % user
                            logger.warn(warnMsg)
                            continue

                    infoMsg = u"获取用户'%s'的密码哈希值" % user
                    logger.info(infoMsg)
        ...
        if not kb.data.cachedUsersPrivileges and isInferenceAvailable() and not conf.direct:
            ...
            for user in users:
                ...
                if Backend.isDbms(DBMS.INFORMIX):
                    ...
                    infoMsg = u"获取用户'%s'的权限数量" % outuser
                    logger.info(infoMsg)
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        warnMsg = u"无法检索用户'%s'的权限数量" % outuser
                        logger.warn(warnMsg)
                        continue

                infoMsg = u"获取用户'%s'的权限" % outuser
                logger.info(infoMsg)
                ...
                if privileges:
                    ...
                    warnMsg = u"无法检索用户'%s'的权限" % outuser
                    logger.warn(warnMsg)

                retrievedUsers.add(user)

        if not kb.data.cachedUsersPrivileges:
            errMsg = u"无法检索数据库用户的权限"
            raise SqlmapNoneDataException(errMsg)
    ...
    def getRoles(self, query2=False):
        warnMsg = u"在 %s 上，角色的概念不存在，sqlmap将用枚举权限代替" % Backend.getIdentifiedDbms()
        logger.warn(warnMsg)
